...
test.describe("Share Functionality", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/");
    await expect(page.locator(".app-spinner-container")).toBeHidden({
      timeout: 30000,
    });
  });
  ...
  test("should copy shareable link to clipboard on Share click", async ({
    page,
    context,
  }) => {
  ...
  test("should load template from shared URL", async ({ page }) => {
    ...
    expect(
      shareableLink,
      "Shareable link should be a non-empty string",
    ).toBeTruthy();
    expect(typeof shareableLink, "Shareable link should be a string").toBe(
      "string",
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to parse shareable link as URL: "${shareableLink}". Error: ${error}`,
      );
    }
