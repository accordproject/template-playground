...
export const prepareSystemPrompt = {
  ...
  explainCode: (editorsContent: editorsContent, aiConfig?: AIConfig, editorType?: 'markdown' | 'concerto' | 'json') => {
    const editorTypeMap = {
    ...
    const prompt = `You are a helpful assistant that explains Accord Project ${typeName} code clearly and concisely.
  ...
  inlineSuggestion: (editorsContent: editorsContent, aiConfig?: AIConfig, editorType?: 'markdown' | 'concerto' | 'json') => {
    const editorName = editorType === 'markdown' ? 'TemplateMark' : editorType === 'concerto' ? 'Concerto' : 'JSON Data';
    const prompt = `You are a helpful assistant that provides inline code completion suggestions for Accord Project
