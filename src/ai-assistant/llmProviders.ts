...
export class AnthropicProvider extends LLMProvider {
  async streamChat(
    ...
    try {
      ...
      await client.messages.stream(params).on('text', (textDelta) => {
        onChunk(textDelta);
      });
...
export class GoogleProvider extends LLMProvider {
  ...
  async streamChat(
    ...
    try {
      // Corrected initialization for @google/genai
      const genAI = new GoogleGenAI({ apiKey: this.config.apiKey });
      const systemInstruction = messages.slice(-2, -1)[0]?.content || '';
      const geminiMessages = this.convertToGeminiFormat(messages);
      
      const generationConfig: any = {}; // Using any to bypass strict GenerateContentConfig issues
      if (this.config.maxTokens) {
