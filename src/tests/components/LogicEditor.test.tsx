...
describe('LogicEditor', () => {
  ...
  it('shows "Show Errors" button when there are errors and editor is not dirty', () => {
    useAppStore.setState({
      editorLogicTs: 'const x = 1;',
      logicTs: 'const x = 1;',
      compilationErrors: [{ message: 'Cannot find name "foo".' }],
    ...
    render(<LogicEditor />);
    expect(screen.getByRole('button', { name: /show errors/i })).toBeInTheDocument();
  });

  it('hides "Show Errors" button when editor is dirty (unsaved changes)', () => {
    useAppStore.setState({
  ...
  it('opens the error modal when "Show Errors" is clicked', async () => {
    useAppStore.setState({
      editorLogicTs: 'const x = 1;',
      logicTs: 'const x = 1;',
      compilationErrors: [{ message: 'Cannot find name "foo".' }],
    ...
    expect(screen.queryByText('Compilation Errors')).not.toBeInTheDocument();

    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /show errors/i }));
    });
