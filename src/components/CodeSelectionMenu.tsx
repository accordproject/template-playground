...
const CodeSelectionMenu: React.FC<CodeSelectionMenuProps> = ({
  ...
  const popupPosition = {
    ...
    top: isExplaining
      ? position.y - 20
      : Math.max(10, Math.min(position.y - 20, window.innerHeight - 300)),
  };
  ...
  if (showExplanation) {
    return (
      ...
      >
        ...
        <div
          className={`max-h-64 pr-6 ${isExplaining ? "overflow-hidden" : "overflow-y-auto"}`}
        >
          {isExplaining ? (
            ...
            <div className="text-sm text-gray-700 prose lg:prose-md">
              <ReactMarkdown
                components={{
                  pre: ({ children }) => {
                    return (
                      <pre className="[&_code]:bg-transparent">{children}</pre>
                    );
                  },
                  code: ({ children, className }) => {
                    return (
                      <code
                        className={`bg-gray-200 p-1 rounded-md before:content-[''] after:content-[''] ${className || ""}`}
                      >
                        {children}
                      </code>
                    );
...
export const useCodeSelection = (
  ...
  const enableCodeSelectionMenu = useAppStore(
    state => state.aiConfig?.enableCodeSelectionMenu ?? true,
  );
    ...
    if (selection && !selection.isEmpty()) {
      ...
      if (selectedText && selectedText.length > 0) {
        ...
        if (editorRect && position) {
          x = Math.max(
            editorRect.left + 20,
            Math.min(position.left, editorRect.right - 150),
          );
          y = Math.max(
            editorRect.top + 20,
            Math.min(position.top, editorRect.bottom - 50),
          );
