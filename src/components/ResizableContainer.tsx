...
const ResizableContainer: React.FC<ResizableContainerProps> = ({
  ...
  const scaledInitialAiChatWidth = hasAiChatPane
    ? initialAiChatWidth * scaleFactor
    : 0;
  ...
  const [rightWidth, setRightWidth] = useState(
    storedRightWidth
      ? Number(storedRightWidth)
      : hasAiChatPane
        ? scaledInitialRightWidth
        : 100 - leftWidth,
  );
  ...
  const [aiChatWidth, setAiChatWidth] = useState(
    hasAiChatPane
      ? storedAiChatWidth
        ? Number(storedAiChatWidth)
        : scaledInitialAiChatWidth
      : 0,
  );
  ...
  const handleLeftDividerResize = useCallback(
    (mouseX: number, containerWidth: number) => {
      if (hasAiChatPane) {
        ...
        if (newLeftWidth >= minLeftWidth && newRightWidth >= minRightWidth) {
          ...
          currentAiChatWidth > minAiChatWidth
        ) {
          ...
          if (aiChatAvailableSpace >= rightPaneDeficit) {
            ...
            const maxLeftPaneWidth =
              leftWidth +
              (aiChatAvailableSpace -
                rightPaneDeficit +
                (newRightWidth - minRightWidth));
  ...
  const handleRightDividerResize = useCallback(
    (mouseX: number, containerWidth: number) => {
      ...
      if (rightPaneAvailableSpace >= aiChatNeededSpace) {
        ...
        if (leftWidth - neededFromLeft >= minLeftWidth) {
          ...
          if (totalAvailableSpace >= minAiChatWidth) {
            ...
            newRightWidth = minRightWidth;
            console.warn(
              "Not enough space to accommodate minimum width for all panes",
            );
          }
        }
      }
      ...
      {!isMobile && (
        <div
          style={{
            ...
            cursor: "col-resize",
            background:
              leftDividerHovered || isDraggingLeft.current ? "#999" : "#ccc",
            position: "relative",
            zIndex: 10,
            userSelect: "none",
            touchAction: "none",
          ...
          {!isMobile && (
            <div
              style={{
                ...
                cursor: "col-resize",
                background:
                  rightDividerHovered || isDraggingRight.current
                    ? "#999"
                    : "#ccc",
                position: "relative",
                zIndex: 10,
