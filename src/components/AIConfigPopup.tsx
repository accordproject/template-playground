...
import MODELS_BY_PROVIDER from "./modelsByProvider";
...
const AIConfigPopup = ({ isOpen, onClose, onSave }: AIConfigPopupProps) => {
  ...
  const [enableInlineSuggestions, setEnableInlineSuggestions] =
    useState<boolean>(true);
  // modelSelect and customModel are used to manage the model selection logic
    ...
    >
      ...
      >
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                {provider &&
                  ...
                  MODELS_BY_PROVIDER[provider].map((modelOption) => (
                    <option key={modelOption.name} value={modelOption.name}>
                      {modelOption.name}{" "}
                      {modelOption.pricing === "free" ? "(Free)" : "(Paid)"}
                    </option>
                  ))}
          ...
          {(modelSelect === "other" || provider === "openai-compatible") && (
            <div>
              <label
                className={`block text-sm font-medium ${theme.label} mb-1`}
              >
              ...
              >
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setEnableCodeSelectionMenu(e.target.checked)
                    }
                    className={`h-4 w-4 rounded focus:ring-2 ${theme.checkbox}`}
                  />
                  <label
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setEnableInlineSuggestions(e.target.checked)
                    }
                    className={`h-4 w-4 rounded focus:ring-2 ${theme.checkbox}`}
                  />
                  <label
