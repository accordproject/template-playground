...
import { FiTerminal, FiShare2, FiSettings, FiMenu } from "react-icons/fi";
      ...
      <div className="playground-sidebar-toggle">
        <Tooltip title={isSidebarExpanded ? "Collapse Sidebar" : "Expand Sidebar"} placement="right">
          <button
            ...
            className="playground-sidebar-toggle-btn"
        ...
        {navTop.map(({ title, icon: Icon, component, onClick, active }) => (
          <Tooltip key={title} title={!isSidebarExpanded ? title : ''} placement="right">
            ...
            >
              ...
              <span className={`playground-sidebar-nav-item-title ${isSidebarExpanded ? 'playground-sidebar-nav-item-title-visible' : ''}`}>
                {title}
              </span>
            </div>
          </Tooltip>
